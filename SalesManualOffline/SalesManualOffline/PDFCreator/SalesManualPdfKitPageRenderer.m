//
//  SalesManualPdfKitPageRenderer.m
//  SalesManualOffline
//
...
@interface SaleManualHtmlPdfKitPageRenderer : UIPrintPageRenderer
...
#pragma mark - SaleManualHtmlPdfKitPageRenderer Implementation

@implementation SaleManualHtmlPdfKitPageRenderer
...
@implementation SalesManualPdfKitPageRenderer
- (id)init {
    if (self = [super init]) {
...
{
   UIPrintFormatter *formatter = _webView.viewPrintFormatter;
    SaleManualHtmlPdfKitPageRenderer *renderer = [[SaleManualHtmlPdfKitPageRenderer alloc] init];
    ...
    //[renderer printFormattersForPageAtIndex:0];
    NSInteger pages = [renderer numberOfPages];
    
    for (int i = 0; i < pages; i++) {
        UIGraphicsBeginPDFPage();
        [renderer drawPageAtIndex:i inRect:renderer.paperRect];
        //[renderer drawContentForPageAtIndex:i inRect:renderer.paperRect];
    }
    ...
    if (strPDFDestinationPath) {
        
       BOOL isPageSaved= [currentReportData writeToFile:strPDFDestinationPath atomically:YES];
        
      if(isPageSaved)
        {
        if ([_delegate respondsToSelector:@selector(htmlPdfKit:didSavePdfFile:)]) {
            [_delegate htmlPdfKit:self didSavePdfFile:strPDFDestinationPath];
        }
      }
    }
